...
class DataServerProgram(BaseProgram):
    ...
    def start(self):
        ...
        self.pm_logger.debug(f"Starting Data Server Program: {self.name}")
        try:
            command = (
            ...
            self.pm_logger.info(f"Started Data Server Program '{self.name}' with PID {self.process.pid}")
            return self.process.pid
        except Exception as e:
            self.pm_logger.error(f"Failed to start Data Server Program '{self.name}': {e}")
            return None
    ...
    def stop(self):
        ...
        """
        self.pm_logger.debug(f"Stopping Data Server Program: {self.name}")
        ...
        if self.default_monitor():
            ...
            try:
                ...
                self.pm_logger.info(f"Waiting for Data Server Program to terminate...")
                time.sleep(60)
                self.pm_logger.info(f"Data Server Program terminated gracefully with SIGINT.")
            except Exception as e:
                self.pm_logger.error(f"Error stopping Data Server Program '{self.name}', pid: {pid}: {e}")

        else:
            self.pm_logger.warning(f"Data Server Program '{self.name}' was not running.")
    ...
    def custom_monitor(self):
        ...
        # if not pid_running:
        #     self.pm_logger.info("Data server not running based on status file. Attempting restart.")
        #     return True
        ...
        try:
            manager.connect()
            self.pm_logger.info("Connection to data server succeeded.")
        except Exception as e:
            self.pm_logger.error(f"[Client] Error connecting to data server: {e}")
            self.pm_logger.info("Attempting restart of data server.")
            return True
